...
int main(int argc, char **argv)
{
    TEST_TAG(asn1);
    {
        ...
        {
            TEST_TAG(string);
            const string ia5({0x16, 0x05, 0x31, 0x32, 0x33, 0x34, 0x35});
            const string octet({0x04, 0x05, 0x31, 0x32, 0x33, 0x34, 0x35});
#define STR "12345"
...

        {
...

            {
                ...
                encoder.clear();
                encoder.append(s);
                const string wanted({0x30,0x08,   0x02,0x01,123,    0x16,0x03,0x34,0x35,0x36});
                TEST_EQ(wanted, encoder.encode());
            }
        }
    }

    {
        ...
        asn1::Decoder decoder;
        {
            ...
            int i;
            {
                ...
                TEST_TRUE(decoder.extract(i));
                TEST_EQ(-128, i);
                decoder.reset({0x02, 0x01, -10});
                TEST_TRUE(decoder.extract(i));
                TEST_EQ(-10, i);

                //Read positive, 2-byte int
                decoder.reset({0x02, 0x02, 0x00, -22});
                TEST_TRUE(decoder.extract(i));
                TEST_EQ(234, i);
