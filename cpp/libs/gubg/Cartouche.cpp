...
namespace gubg { 
    ...
    Cartouche::Cartouches Cartouche::parse(const string &content)
    {
        ...
        for (auto &l: line::split(content))
        {
            ...
            if (!current)
            {
                LL("No cartouche detected yet");
                const auto ix = l.txt.find(marker);
                if (ix == string::npos)
                    //Still no cartouche detected
                    continue;
                LL("We detected a cartouche, add one and initialize it");
            ...
            {
                LL("We are detecting a cartouche");
                ...
                LL("This line is part of the cartouche, add it, together with its endline marker");
