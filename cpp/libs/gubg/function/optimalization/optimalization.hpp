...
namespace gubg
{
    template <typename FunctionT>
        class Optimizer
        {
                ...
                typedef class: public Block<Optimizer>
            {
                ...
                template <typename B>
                    bool optimize(typename FunctionT::InputType &input, FunctionT &function, typename FunctionT::BaseType *res, B *block)
                    {
                        ...
                        long lMaxSteps = nrInputs*nrSteps_;
                        ...
                        {
                            // cout << endl << "SCG step " << lStep << " of (" << lMaxSteps << "=" << nrSteps_ << "*" << nrInputs << ")" << endl; // do(2)
                            ...
                            *pAvailableInput = *pInputStore;
                            ...
                            if (bSuccess)
                            {
                                ...
                                if ((dAlpha*Cube::l2Norm(correction) <  roundOffError_) && (abs(dFnew-dFold) <  roundOffError_))
                                {
                                    if (block)
                                        continue;
                                    else
                                ...
                                } else
                                {
                                    ...
                                    if (Cube::l2Norm(gradnew) <= roundOffError_)
                                    {
                                        if (block)
                                            continue;
                                        else
            ...
            private:
                const double paramEps_;
                const double roundOffError_;
                const long nrSteps_;
        };
}
