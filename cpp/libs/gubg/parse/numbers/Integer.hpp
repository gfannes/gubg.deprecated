...
namespace gubg
{
    namespace parse
    {
        namespace numbers
        {
            ...
            inline bool read_(long &l, const char *buf, size_t len, tmp::IntType<4>)
            {
                SS(buf, len);
                l = 0;
...
#undef L_CASE
            ...
            inline bool read_(long &l, const char *buf, size_t len, tmp::IntType<8>)
            {
                SS(buf, len);
                l = 0;
#define L_TABLE(n,m) const static unsigned long long lt_ ## n[] = {m*0,m*1,m*2,m*3,m*4,m*5,m*6,m*7,m*8,m*9};
...
#undef L_CASE
	    ...
	    {
		    return read_(l, buf, len, tmp::IntType<sizeof(long)>());
	    }
