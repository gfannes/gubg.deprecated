...
class Visu
{
	...
	Line addLine(real[] sco, real[] eco)
		{
			auto line = new Line(sco, eco);
			_scene.addStyledDrawable(line);
			return line;
		}

	Circle addCircle(real[] center, real radius)
		{
			auto circle = new Circle(center, radius);
			_scene.addStyledDrawable(circle);
			return circle;
		}
	...
	Rectangle addRectangle(real[] lb, real[] tr)
		{
			...
			_scene.addStyledDrawable(rectangle);
			return rectangle;
		}
	Rectangle addCenteredRectangle(real[] center, real[] wh){return addRectangle([center[0]-0.5*wh[0], center[1]-0.5*wh[1]], [center[0]+0.5*wh[0], center[1]+0.5*wh[1]]);}
	Rectangle addCenteredSquare(real[] center, real side){return addRectangle([center[0]-0.5*side, center[1]-0.5*side], [center[0]+0.5*side, center[1]+0.5*side]);}
