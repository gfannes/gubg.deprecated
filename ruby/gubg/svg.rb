...
module SVG
    ...
    #A text shape
    #In the style ctor argument, you can specify fontSizeReference as :x or :y, else it will be the avg between x and y
    class Text < Shape
        @@defaultStyles[Text] = {
        ...
        def initialize(text, x, w, style = nil)
            @text, @x, @y, @style = escape_(text), x, w, style
        end
        def generate(parent, transform)
            x, y = transform.coordinates(@x, @y)
